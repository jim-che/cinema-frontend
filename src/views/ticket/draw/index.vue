...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除出票管理编号为"' + ids + '"的数据项？').then(function() {
        return delDraw(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('ticket/draw/export', {
        ...this.queryParams
