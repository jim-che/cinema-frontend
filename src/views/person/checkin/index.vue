<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['person:checkin:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['person:checkin:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['person:checkin:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="checkinList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="出勤次数" align="center" prop="attendanceTime" />
      <el-table-column label="出勤率" align="center" prop="attendanceRate" >
        <template slot-scope="scope">
          <span>{{scope.row.attendanceRate + '%'}}</span>
        </template>
      </el-table-column>
      <el-table-column label="休假时间" align="center" prop="daysOff" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="休假时间" prop="daysOff">
          <el-input v-model="form.daysOff" placeholder="请输入休假时间" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        attendanceTime: [
          { required: true, message: "出勤次数不能为空", trigger: "blur" }
        ],
        attendanceRate: [
          { required: true, message: "出勤率不能为空", trigger: "blur" }
        ],
        daysOff: [
          { required: true, message: "休假时间不能为空", trigger: "blur" }
        ],
        isDeleted: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除考勤管理编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('person/checkin/export', {
        ...this.queryParams
